...
import { CHANNEL_MOCKS } from "../config/channel-mock" // Ваш файл с моками
import { SUBSCRIBER_RANGES } from "../config/filter-options" // Конфиг диапазонов

// --- Local Components ---
import { FilterButton } from "./filter-button"
import { FilterModal } from "./filter-modal"

// --- Helper: Парсинг подписчиков ("2млн" -> 2000000) ---
    ...
    return (
        <section className="mt-[106px] mb-[110px] px-7.5 relative">
            ...
            <div className="flex justify-between items-center mb-9">
                ...
                {/* Правая часть: Свайпер тегов + Кнопка */}
                {/* ml-4 отталкивает блок от счетчика */}
                    ...
                    {filterCategories.length > 0 && (
                        <div className="min-w-0 max-w-full">
                            <Swiper
                                ...
                                freeMode={true}
                                mousewheel={true}
                                dir="rtl" // ВАЖНО: Прижимает слайды вправо к кнопке
                                className="w-full"
                            >
                                {filterCategories.map(cat => (
                                    // !w-auto позволяет слайду занимать ширину текста
                                    <SwiperSlide key={cat} className="!w-auto">
                                        <button 
                                            dir="ltr" // Возвращаем направление текста внутри кнопки
                                            onClick={() => removeCategoryTag(cat)}
                                            className="px-4 py-2 rounded-full border border-white/30 text-white text-sm hover:bg-white/10 transition-colors whitespace-nowrap"
                                        >
                                            {cat}
                                        </button>
                                    </SwiperSlide>
                                ...
                                <>
                                    ...
                                    <FilterModal 
                                        ...
                                        onSave={(cats, rangeIdx) => {
                                            ...
                                            setVisibleCount(BATCH_SIZE); // Сброс пагинации
                                        }}
                                    />
                                </>
                            )}
            ...
            {currentChannels.length > 0 ? (
                <div className="grid grid-cols-3 gap-x-4 gap-y-5 mb-15">
                    {currentChannels.map((channel, idx) => (
                        // Используем idx в ключе на случай дублей в моках при разработке
                ...
                <div className="flex flex-col items-center justify-center py-20 text-white/50">
                    ...
                    <button 
                        ...
                        className="mt-4 text-[#0088FF] hover:underline"
                    >
                        Сбросить фильтры
                    </button>
                </div>
